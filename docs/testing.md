...
每次结构或样式调整至少检查 1920×1080、1440×900、1366×768 和 960×720，并检查可调宽度侧栏、组合筛选、分组、单图/胶片栏、扫描/语义任务、错误、模型不可用、图片选中、详情与焦点。相邻主/次按钮操作组还必须检查高度、圆角、边框、基线和间距一致，至少覆盖设置页底部操作组的深色、白天和窄窗口状态。正式基准图保存到 `docs/screenshots/semantic-*.png`。production build 后确认确定性夹具模块只存在于开发动态分支。
