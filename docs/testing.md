...
- 首次导入在没有可用内嵌预览时仍需一次源像素 decode，以生成 `grid-640-v1`；完整 BLAKE3 指纹仍保留。JPEG 有有效 EXIF 内嵌缩略图时优先使用它。
...
- 新导入图像处理使用有界的 2–4 个 worker；语义批次上限为 32，批量结果一次事务写入；数据库写入、归属解析和进度持久化保持串行。
- 隔离 release 基准（2 张 4000×3000 JPEG）：冷导入 360 ms（源 decode 223 ms、resize 117 ms）；缓存复用 204 ms（源 decode 0、缩略图 decode 13.6 ms）；暖扫描 141 ms 且图像处理为 0。
- release TinyCLIP 48 张仓库 PNG：批次 8 为 76.8 张/秒，批次 32 为 87.7 张/秒；这是模型吞吐 smoke，不代表真实摄影分类质量。
