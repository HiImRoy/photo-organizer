...
- [src/components/DetailPanel.tsx](E:/Code/Codex/photo-organizer/src/components/DetailPanel.tsx) 显示 Auto/Manual/Effective/provenance 并支持五类 Registry 字段修正；分类修正区域默认折叠，展开后使用中文下拉控件和与侧边栏一致的主色色块多选，不接受原始分类 ID 文本输入；[src/App.tsx](E:/Code/Codex/photo-organizer/src/App.tsx) 提供同样使用选项控件的批量修正入口。
