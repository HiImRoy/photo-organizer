...
本阶段原计划统一当前图片状态、预览资源层级和异步加载安全。当前按用户体验要求保留旧预览链路，仅保留 `activeAssetId` 单一当前图片状态和“双击回到适应屏幕”的交互调整；新的 DPR/tier 资源方案不作为已完成内容。导入、分析和模型 decode 遵循后续缩略图-only 硬约束；本文件中的 original 仅指用户主动查看预览的显式例外。
